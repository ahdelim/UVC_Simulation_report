subroutine optimization_exergy(theta_1, v_com, v_suc, p_scv, p_dcv, t_scv, t_dcv, s_scv, s_dcv, h_scv, h_dcv, u_scv, u_dcv, m_scv, m_dcv, dmdtheta1_si, dmdtheta1_so, dmdtheta1_leak_s, dmdtheta1_di, dmdtheta1_do, dqdtheta_sc, dqdtheta_dc, Q_dcsc_ex, Q_schc_ex, Q_scro_ex, Q_schc_vd_ex, Q_scoil_vd_ex, Q_dchc_ex, Q_dcro_ex, Q_dchc_vd_ex, Q_dcoil_vd_ex, L_f_vs, L_ef_vh, L_s_vh, L_ef_ro, L_lub, P_bear_s, L_lip_seal)
    ...
    do i = 1, no_data+1
        ...
        I_thr_leak(i) = abs(dmdtheta1_leak_s(i)*(s_scv(i) - s_dcv(i) - (h_scv(i) - h_dcv(i))/t_scv(i))*omega_1)          ! [W] throttling lost of leakage  (total)
        ! ------------------------------
    ...
    enddo
        ...
        ! ---------------------------------------------
        !write(161,2981), "Degree","I_thr_scv[W]", "I_thr_dcv[W]", "I_thr_leak[W]", "I_ht_scv[W]", "I_ht_dcv[W]", "I_ef_vh[W]", "I_s_vh[W]", "I_f_vs[W]", "I_ef_ro[W]", "I_lub[W]"
        !do i = 1, no_data+data_step, data_step
        !    write(161,2982), theta_1(i)*180.0/pi, I_thr_scv(i), I_thr_dcv(i), I_thr_leak(i), I_ht_scv(i), I_ht_dcv(i), I_ef_vh(i), I_s_vh(i), I_f_vs(i), I_ef_ro(i), I_lub(i)
        !enddo
        ! ---------------------------------------------
        ! Write on screen
        ! ---------------------------------------------
        ...
        !write (113,*) " -------------------------------------------------------------------- "
        !write (113,2990) "Total Mean Exergy Supplied Rate                exergy_sup          = ", exergy_sup, " W"
        ...
        !write (113,2990) "Cubic Feet per Minute (CFM)                               = ", mass_total/rho_suc*35.3147*60, " CFM"
        !
...
endsubroutine optimization_exergy