subroutine oil_lub_network(theta_1, theta_2, p_dcv, p_scv, eccr)
    ...
    R1 = (8.*miu_oil*L_feed_lb*1.e-3)/(pi*((0.5*dia_feed_lb*1.e-3)**4))
    R2 = (8.*miu_oil*L35*1.e-3)/(pi*((0.5*dia_feed_vert*1.e-3)**4))
    R3 = (8.*miu_oil*L45*1.e-3)/(pi*((0.5*dia_feed_vert*1.e-3)**4))
    R4 = (8.*miu_oil*L_feed_radial_jour*1.e-3)/(pi*((0.5*dia_feed_radial_jour*1.e-3)**4))
    del_P4 = 0.5*rho_oil*(omega_1**2)*(((r_shaft*1.e-3)**2) - ((0.5*dia_feed_vert*1.e-3)**2))     ! Pressure difference across path 5-6
    ...
    R6 = (8.*miu_oil*L_feed_radial_jour*1.e-3)/(pi*((0.5*dia_feed_radial_jour*1.e-3)**4))
    del_P6 = 0.5*rho_oil*(omega_1**2)*(((r_shaft*1.e-3)**2) - ((0.5*dia_feed_vert*1.e-3)**2))       ! Differential pressure across path 4-7
    ...
    do i = 1, no_data+1
        ...
        !Path 1-2-3-4-5-6-7-8-9-c
        mat_Q_element(13,2) = 0.5*R1 + R2 + R7
        mat_Q_element(13,3) = 0.5*R4 + R3
        mat_Q_element(13,4) = R5
        mat_Q_element(13,10) = R9
        mat_Q_element(13,13) = R10c
        ...
        !mat_Q_element(15,3) = R3 + 0.5*R4      ! if add in path 5-6
        !mat_Q_element(15,4) = R5               ! if add in path 5-6
        ...
        mat_B(7) = (p_dcv(i) - p_scv(i))*1000.
        mat_B(11) = rho_oil*g_grav*(L12-L34)*1.e-3 + del_P6 + del_P7 - p_dcv(i)*1000. + p_disc*1000.
        mat_B(12) = rho_oil*g_grav*(L12-L34)*1.e-3 + del_P4 + del_P7 - p_dcv(i)*1000. + p_disc*1000.    !path 1-2-3-4-5-6-7-8-c
        !mat_B(12) = rho_oil*g_grav*(-L34 + l_com + l_bearing)*1.e-3 + del_P4 + del_P7 - del_P11        !path 3-4-5-6-7-8-9-10-3
        !mat_B(13) = rho_oil*g_grav*(L12-L34 + l_com)*1.e-3 + del_P6 + del_P7 - p_dcv(i)*1000. + p_disc*1000.   !Path 1-2-3-4-7-8-9-10-c
        mat_B(13) = rho_oil*g_grav*(L12-L34 + l_com)*1.e-3 + del_P7 - p_scv(i)*1000. + p_disc*1000.            !path 1-2-3-4-5-6-7-8-9-c 
        mat_B(14) = - del_P4 + del_P6     ! Path 4-5-6-7-4
        mat_B(15) = rho_oil*g_grav*(-L34 + l_com + l_bearing)*1.e-3 + del_P6 + del_P7 - del_P11     ! Path 3-4-7-8-9-10-c
        !mat_B(15) = rho_oil*g_grav*(-L34 + l_com + l_bearing)*1.e-3 + del_P4 + del_P7 - del_P11     ! Path 3-4-5-6-7-8-9-10-c
