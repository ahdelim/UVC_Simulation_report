subroutine leakage_fanno_width_s_clr(T_d, p_e, k_ratio, mach_unity, R_specific, miu_refrigerant, length_flow, error_press, leak_conv_criterion, press_ratio, mach_t, clearance, l_fanno_eq, mach_e, dmdtheta_leak)
    ! ----------------------------
...
!102         continue
            ...
            elseif (pt_pe < press_ratio .and. abs(error_press) > leak_conv_criterion) then
                    ...
                    if (mach_t > mach_t_max+0.01 .or. mach_t < 0.0) then
                        !mach_t = 0.00000001
                        !mach_t = 0.2999
                        mach_t = mach_t_max+0.005 - abs(error_press)*0.000001
...
return    
2982 format (5F8.4) 
endsubroutine leakage_fanno_width_s_clr
