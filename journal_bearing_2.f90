subroutine journal_bear_hirani_2(theta_1, dgammadt_1, F_cx, F_cy, F_resultant, eccr, att, h_min, p_max, Q_martin, cl_rotor_rad_dy)
    implicit none
    ...
    double precision, dimension (1:no_data+1) :: omega_av, theta_a1, theta_a2, omega_av_ro
    ...
    double precision, dimension (1:N_DATA+1) :: eccr_ite_ro, att_ite_ro, deccrdt_ite_ro, dattdt_ite_ro
    double precision, dimension (1:p_data+1) :: p_max_dis
    double precision k_for_M, term_const_F, f1, f2, H, B1, term_const_F_ro          ! f1, f2, H, B1 refers to Hirani paper
    ...
    double precision, dimension (1:no_data+1) :: eccr_ro, att_ro, deccrdt_ro, dattdt_ro
    ...
    double precision, dimension (1:no_data+1) :: cl_bearing_dy, cl_rotor_rad_dy
    double precision, dimension (1:no_data+1) :: arg_F_result, omega_F_result, omega_F_result_ro
    ! --------------------------------------------------------------------
    ...
    do i = 1, no_data+1
        ...
        omega_av_ro(i) = 0.5*(omega_1 + dgammadt_1(i)) - omega_F_result(i)
    enddo
    
    ! ----- Calculation for constant dummy term --------------
    term_const_F = (cl_bear_s**2)/(6.*miu_oil*l_bearing*r_bearing**3)*1.d12
    term_const_F_ro = (cl_rad_roll**2)/(6.*miu_oil*l_ecc*r_ecc**3)*1.d12
...
! --------------------------------------------------------=----------------------------------------------------------------------- !
    ...
    do while (error_eccr > conv_criterion .or. error_att > conv_criterion)
        do k = 1, N_DATA
            if (k == 1) then
                eccr_ite_ro(k) = eccr_guess
                att_ite_ro(k) = att_guess
                dem_theta_a1(i) = eccr_ite_ro(k)*(omega_av_ro(i) - dattdt_guess)  ! denominator of the theta 1 equation
                if (dem_theta_a1(i) == 0. .and. deccrdt_guess > 0.) then
                    theta_a1(i) = 0.5*pi
                elseif (dem_theta_a1(i) == 0. .and. deccrdt_guess < 0.) then
                    theta_a1(i) = - pi/2.
                ...
                dem_theta_a1(i) = eccr_ite_ro(k)*(omega_av_ro(i) - dattdt_ite_ro(k-1))
                if (dem_theta_a1(i) == 0. .and. deccrdt_ite_ro(k-1) > 0.) then
                    theta_a1(i) = pi/2.
                elseif (dem_theta_a1(i) == 0. .and. deccrdt_ite_ro(k-1) < 0.) then
                    theta_a1(i) = -pi/2.
                elseif (dem_theta_a1(i) == 0. .and. deccrdt_ite_ro(k-1) == 0.) then
                    theta_a1(i) = 0.0
                else 
                    theta_a1(i) = atan( deccrdt_ite_ro(k-1) / dem_theta_a1(i) )
                endif
            ...
            ! by using Weddle's formula
            ! ------------------------------------
            call journal_mecc_matt(eccr_ite_ro(k), att_ite_ro(k), theta_a1(i), k_for_M, l_ecc, r_ecc, M_ecc, M_att)
            
            deccrdt_ite_ro(k) = F_resultant(i)*term_const_F_ro*M_ecc          ! calculation of deccrdt (gradient) based on current eccr and attitude ! 0.5 accounts for the resultant force (0.5 for upper bearing and 0.5 for lower bearing)
            dattdt_ite_ro(k) = F_resultant(i)*term_const_F_ro*M_att/eccr_ite_ro(k) + omega_av_ro(i)
            
            eccr_ite_ro(k+1) = eccr_ite_ro(k) + deccrdt_ite_ro(k)*(theta_1(i+1) - theta_1(i))/omega_1/N_DATA     ! Eccentric Ratio (calculate for next iteration)
            att_ite_ro(k+1) = att_ite_ro(k) + dattdt_ite_ro(k)*(theta_1(i+1) - theta_1(i))/omega_1/N_DATA         ! Attitude angle    (next), time_step = time / N_DATA, time is /\theta_1//\omega_1
            
            if (i == no_data+1) then
                eccr_ite_ro(k+1) = eccr_ite_ro(k) + deccrdt_ite_ro(k) * ( theta_1(i) - theta_1(i-1) ) / omega_1/N_DATA      ! Eccentric Ratio (next)
                att_ite_ro(k+1) = att_ite_ro(k) + dattdt_ite_ro(k) * ( theta_1(i) - theta_1(i-1) ) / omega_1/N_DATA         ! Attitude angle    (next)       
            endif
            
            ! Make the attitude angle below 360degree
            if (att_ite_ro(k+1) > 2.0*pi) then
                n = floor(att_ite_ro(k+1)/(2.0*pi))
                att_ite_ro(k+1) = att_ite_ro(k+1) - n*2.0*pi
            elseif (att_ite_ro(k+1) < - 2.0*pi) then
                n = floor(abs(att_ite_ro(k+1))/(2.0*pi))
                att_ite_ro(k+1) = - (abs(att_ite_ro(k+1)) - n*2.0*pi)
                !att_ite(k+1) = att_ite(k+1) + n*2.*pi
            endif
        enddo

        error_eccr = abs(eccr_ite_ro(N_DATA) - eccr_ite_ro(1))/eccr_ite_ro(1)
        error_att = abs(att_ite_ro(N_DATA) - att_ite_ro(1))/att_ite_ro(1)
        if (i == no_data/2) then
            print *, ' ---------------------------------------------------------------------------- '
            print '(2x,A,F10.2,A)', 'Error in Eccentricity Ratio = ', error_eccr*100.0, ' % '
            print '(2x,A,F10.2,A)', 'Error in Attitude Angle     = ', error_att*100.0, ' % '
        ...
        ! ------ assign last point to the first point (guess point) of next iteration if not yet converged ------
        eccr_guess = eccr_ite_ro(N_DATA)
        deccrdt_guess = deccrdt_ite_ro(N_DATA)
        att_guess = att_ite_ro(N_DATA)
        dattdt_guess = dattdt_ite_ro(N_DATA)
        
    enddo
172         continue 
        ! ------ store the converged point ------- 
        eccr_ro(i) = eccr_ite_ro(N_DATA)
        deccrdt_ro(i) = deccrdt_ite_ro(N_DATA)
        att_ro(i) = att_ite_ro(N_DATA)
        dattdt_ro(i) = dattdt_ite_ro(N_DATA)
    enddo
    ! ------------------------------------------
    ! Part 3: Calcualte P_max and h_min
    ! 1     . Determine necessary dummy variables to find theta_P_max       
    ...
    theta_a12_dis = theta_a1(i)     
    do j = 1, p_data     
        f1 = eccr_ro(i)*(omega_av_ro(i) - dattdt_ro(i))*sin(theta_a12_dis) - deccrdt_ro(i)*cos(theta_a12_dis)
        f2 = (1.0 + eccr_ro(i)*cos(theta_a12_dis))**3
        H = 1.0 + eccr_ro(i)*cos(theta_a12_dis)
        B1 = H*(1.0 + H)/(l_ecc*1.d-3/(r_ecc*1.d-3))**2
        p_max_dis(j) = 1.5*miu_oil*(l_ecc*1.d-3/cl_rad_roll)**2*f1/((1.0 + 1.0/(4.*B1))*f2)
        theta_a12_dis = theta_a12_dis + (theta_a2(i) - theta_a1(i))/p_data
    enddo
    h_min(i) = cl_rad_roll*(1.0-eccr_ro(i))  ! radial, not diametral
    p_max(i) = maxval(p_max_dis)
    enddo
    ...
    p_supply = p_disc - p_suc       ! [kPa] supply pressure -- assume to be pressure difference between discharge and suction chamber
    do i = 1, no_data + 1
        Q_hydro(i) = 2.*cl_rad_roll*l_ecc*r_ecc*1.d-6*(eccr_ro(i)*(dattdt_ro(i) - omega_av_ro(i))*cos(theta_a1(i)) - deccrdt_ro(i)*sin(theta_a1(i)))      ! hydrodynamic flow
        Q_hydro(i) = abs(Q_hydro(i))    ! [m3/s]
        Q_press(i) = pi*p_supply*1000.0*r_ecc*1.d-3*cl_rad_roll**3/(6.0*miu_oil*l_ecc*1.d-3)*(1.0 + 1.5*eccr_ro(i)**2)     ! [m3/s] pressure driven feed flow
        Q_martin(i) = Q_hydro(i) + Q_press(i) - 0.3*(Q_hydro(i)*Q_press(i))**0.5       ! [m3/s] Martin Flow (empirical equation), Martin (1993)
    enddo
    ...
    !print *, ' ---------------------------------------------------------------------------- '  
    !write(6,2989) " Maximum Pressure (ecc)                  = ", maxval(p_max)/1.d6, " MPa"
    !write(6,2989) " Minimum Oil Film Thickness (ecc)        = ", minval(h_min)*1.d6, " micron-m"
    !write(6,2989) " Minimum Inst Required Oil Flow (ecc)    = ", maxval(Q_martin)*1.d6, " cc/s"
    !print *, ' ---------------------------------------------------------------------------- '
    
    
    ! ------------- Evaluate dynamic clearance
    do i = 1, no_data+1
        cl_rotor_rad_dy(i) = cl_rad_ro - cl_rad_roll*eccr_ro(i)*cos(theta_1(i) - arg_F_result(i) - att_ro(i)) - cl_bear_s*eccr(i)*cos(theta_1(i) - arg_F_result(i) - att(i))
        if (cl_rotor_rad_dy(i) <= 0.0) then
            cl_rotor_rad_dy(i) = 0.1*cl_rad_ro
        endif
        !if (theta_1(i)*180.0/pi >= 180.0 .and. theta_1(i)*180.0/pi <= 260.0) then
        !    cl_rotor_rad_dy(i) = 3.0*cl_rotor_rad_dy(i)
        !endif
...
2989 format (2x,A,F12.4,A)  
    endsubroutine journal_bear_hirani_2
