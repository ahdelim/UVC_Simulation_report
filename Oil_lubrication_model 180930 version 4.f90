subroutine oil_lub_network_ver4(theta_1, theta_2, p_dcv, p_scv, eccr)
    implicit none
    ...
    double precision dia_feed_vert, cl_gap_ecc, num_R1        ! diameter of shaft feeding hole of path 3-4-5 ! eccentric clearance gap ! number of R1 
    ...
    ! ------ input for matrix ------ !
    integer, parameter :: n = 16 !n = 9 if it is 9 equations 9 unknowns (version 1-0)
    ...
    read(116,*) cl_gap_ecc  ! [mm] clearance between eccentric and 
    read(116,*) num_R1      ! Number of lower bearing feeding holes
    p_reservoir = p_disc
    close(116)
    ! ------------------------
    ...
    ! ------------------------
    L45 = L35 - L34     ! [mm] Length of path 4-5 = path 6-7
    ...
    R2 = (8.*miu_oil*L34*1.d-3)/(pi*((0.5*dia_feed_vert*1.d-3)**4))
    ...
    R7 = 6.*miu_oil*log(r_average_rolleredge/r_shaft)/(pi*(cl_gap_ecc*1.d-3)**3)  ! prototype version 2-3, large eccentric height
    !R7 = 6.*miu_oil*log(r_max_rolleredge/r_shaft)/(pi*(cl_upend)**3)  ! prototype version 2-3, large eccentric height
    ...
    R11 =  6.*miu_oil*log(r_average_rolleredge/r_shaft)/(pi*(cl_gap_ecc*1.d-3)**3)  ! prototype version 2-3, large eccentric height
    !R11 =  6.*miu_oil*log(r_max_rolleredge/r_shaft)/(pi*(cl_lowend)**3)  ! prototype version 2-3, large eccentric height
    ...
    do i = 1, no_data+1
        ...
        R10c = 6.*miu_oil*log(r_ro/r_average_rolleredge)/(pi*cl_lowend**3)*(2.*pi/(2.*pi - theta_2(i) - 0.00000001))
        R10s = 6.*miu_oil*log(r_ro/r_average_rolleredge)/(pi*cl_lowend**3)*(2.*pi/(theta_2(i) + 0.00000001))

        R12 = 12.*miu_oil*L310*1.d-3/((cl_bear_s**3)*pi*r_bearing*1.d-3*(1. + 1.5*eccr(i)**2))
        ...
        mat_Q_element(2,14) = 1.0
        mat_Q_element(2,15) = 1.0
        ...
        mat_Q_element(5,6) = -1.0
        mat_Q_element(5,7) = -1.0
        mat_Q_element(5,8) = -1.0
        mat_Q_element(5,9) = -1.0
        mat_Q_element(5,10) = -1.0
        
        !Equation 6 (node 10)
        mat_Q_element(6,13) = 1.0
        mat_Q_element(6,14) = -1.0
        mat_Q_element(6,15) = -1.0
        mat_Q_element(6,16) = 1.0
        
        !Equation 7 KVL on Working Chamber, Upper End Face clearance
        mat_Q_element(7,6) = -R8c
        mat_Q_element(7,7) = R8s
        
        !Equation 8 KVL on Working Chamber, Lower End Face clearance
        mat_Q_element(8,11) = -R10c
        mat_Q_element(8,12) = R10s
        
        ! -------------------------
        ! Equation 9 is cancelled in version 2-3 prototype
        !Equation 9 KVL on Spiral Groove, upper journal bearing clearance
        !mat_Q_element(9,4) = -R5
        !mat_Q_element(9,5) = Rsg5
        ! -------------------------
        
        !Equation 9 KVL on Spiral Groove, Eccentric-Piston bearing clearance       
        mat_Q_element(9,7) = -R9
        mat_Q_element(9,8) = Rsg9
    
        !Equation 10 KVL on Spiral Groove, Lower journal bearing bearing clearance               
        mat_Q_element(10,14) = -R12
        mat_Q_element(10,15) = Rsg12
        
        !Equation 11 path 1-2-3-4-7-8-c Surface to Compression Chamber via upper end face
        mat_Q_element(11,1) = R1/num_R1
        mat_Q_element(11,2) = R2 + R7
        mat_Q_element(11,5) = 0.5*R6
        mat_Q_element(11,6) = R8c

        !Equation 12 path 1-2-3-4-7-8-9-c Surface to Compression Chamber via lower end face
        mat_Q_element(12,1) = R1/num_R1
        mat_Q_element(12,2) = R2 + R7
        mat_Q_element(12,5) = 0.5*R6
        mat_Q_element(12,8) = R9
        mat_Q_element(12,11) = R10c
        
        !Equation 13 path 3-4-7-8-9-10-3
        mat_Q_element(13,2) = R2 + R7
        mat_Q_element(13,5) = 0.5*R6
        mat_Q_element(13,8) = R9
        mat_Q_element(13,13) = R11
        mat_Q_element(13,14) = R12

        !Equation 14 Path 4-5-6-7-4
        mat_Q_element(14,3) = R3 + 0.5*R4
        mat_Q_element(14,4) = R5
        mat_Q_element(14,5) = -R6/2.
        ! Equation 15 if Q6 is taken out
        !mat_Q_element(14,5) = 1.0
        
        !mat_Q_element(16,6) = 1.0
        mat_Q_element(15,10) = 1.0
        mat_Q_element(16,16) = 1.0
        
        
        mat_B(7) = (p_dcv(i) - p_scv(i))*1000.  !Pc - Ps
        mat_B(8) = (p_dcv(i) - p_scv(i))*1000.
        !mat_B(9) = rho_oil*g_grav*(L35-L34)*(1.d-3)*(cos(angle_sg) - 1.0)
        mat_B(9) = rho_oil*g_grav*(l_com)*(1.d-3)*(cos(angle_sg_eccentric) - 1.0)
        mat_B(10) = rho_oil*g_grav*(l_bearing)*(1.d-3)*(cos(angle_sg_lower) - 1.0)        ! prototype ver 2-3 stop using
        mat_B(11) = rho_oil*g_grav*(L12-L34)*1.d-3 + del_P4 + del_P7 - p_dcv(i)*1000. + p_reservoir*1000.
        mat_B(12) = rho_oil*g_grav*(L12-L34 + l_com)*1.d-3 + del_P7 - p_dcv(i)*1000. + p_reservoir*1000. + del_P4 !Path 1-2-3-4-7-8-9-10-c
        mat_B(13) = rho_oil*g_grav*(-L34 + l_com + l_bearing)*1.d-3 + del_P7 - del_P11 + del_P4    ! Path 3-4-7-8-9-10-3
        mat_B(14) = del_P4 - del_P6           ! Path 4-5-6-7-4
        !mat_B(14) = 0.0     ! Q6 = 0
        !mat_B(14) = 0.0 !0.5*area_sg*r_shaft*1.d-3*omega_1*sin(angle_sg)
        mat_B(15) = 0.5*area_sg_ecc*r_roi*1.d-3*omega_1*sin(angle_sg_eccentric) ! ------ ECCENTRIC ROTATION
        mat_B(16) = 0.5*area_sg_lower*r_shaft*1.d-3*omega_1*sin(angle_sg_lower)
    ...
    ! ----------------------------------
    write (141,2981), "Degree", "Q1[m3/s]", "Q2[m3/s]", "Q3[m3/s]","Q4[m3/s]", "Q6[m3/s]", "Q7c[m3/s]", "Q7s[m3/s]", "Q8[m3/s]", "Q9a[m3/s]", "Q9b[m3/s]", "Q10c[m3/s]", "Q10s[m3/s]", "Q11[m3/s]", "Q12[m3/s]", "Q13a[m3/s]", "Q13b[m3/s]"
    write (142,2981), "Degree", "P1[Pa]", "P2[Pa]", "P3[Pa]", "P4[Pa]", "P5[Pa]", "P6[Pa]", "P7[Pa]", "P8[Pa]", "P9[Pa]", "P10[Pa]"
    do i = 1, no_data+data_step, data_step
        
        write(141,2982) theta_1(i)*180.0/pi, Q1(i), Q2(i), Q3(i), Q4(i), Q6(i), Q7c(i), Q7s(i), Q8(i), Q9a(i), Q9b(i), Q10c(i), Q10s(i), Q11(i), Q12(i), Q13a(i), Q13b(i)
